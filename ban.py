...
print("Github : https://github.com/xyepatalo" + "\n")
...
def main(token, group_id):
    try:
        ...
        print("Обнаружен пост, жду набор комментариев с цп")
        print("Проверка поста -- "+str(wall['items'][0]['id']))
        ...
        while True:
            ...
            for group_id, latest_post_id in dict_of_last_id.items():
                if post_id > latest_post_id:
                    print("Обнаружен пост, жду набор комментариев с цп")
                    print("Проверка поста -- "+str(post_id))
                    list_id = []
...
def get_comments(vk, group_id, post_id, list_id, if3):
        ...
        print("Всего комментариев "+str(count))             # получаем кол-во комментариев
...
def check_video(vk, group_id, title, comment_id, id, description, owner_id, video_id, list_id, if3):
    ...
    if str(id) not in list_id:
        if x > 0:
                ...
                info = datetime.now().strftime("%H:%M:%S")+ " [в названии видео, "+ f_name + " " + l_name +"] Зарепортил[+] из-за: "+bad_found
                print(info)
                ...
                if id > 651000000:
                        ...
                        f_name = ug[0]['first_name']
                        l_name = ug[0]['last_name']
                        info = datetime.now().strftime("%H:%M:%S")+ " [видео, "+ f_name + " " + l_name +"] Зарепортил[+] из-за недавно созданной страницы"
                        print(info)
                        log_to_tg(info, None, 'https://vk.com/video'+str(owner_id)+'_'+str(id))
                else:
                    ...
                    if users_get[0]['has_photo'] == 0:
                            ...
                            f_name = ug[0]['first_name']
                            l_name = ug[0]['last_name']
                            info = datetime.now().strftime("%H:%M:%S")+ " [видео, "+ f_name + " " + l_name +"] Зарепортил[+] из-за отсутствия авы"
                            print(info)
                            log_to_tg(info, None, 'https://vk.com/video'+str(owner_id)+'_'+str(id))
                    else:
                        if if3 == 0:
                            ...
                            if users_get[0]['online'] == 0:
                                    ...
                                    f_name = ug[0]['first_name']
                                    l_name = ug[0]['last_name']
                                    info = datetime.now().strftime("%H:%M:%S")+ " [видео, "+ f_name + " " + l_name +"] Зарепортил[+] из-за оффлайна(возможно взломанный акк или бот)"
                                    print(info)
                                    log_to_tg(info, None, 'https://vk.com/video'+str(owner_id)+'_'+str(id))
                            else:
                                x = 0 
                                ...
                                if x > 0:
                                    ...
                                    f_name = ug[0]['first_name']
                                    l_name = ug[0]['last_name']
                                    info = datetime.now().strftime("%H:%M:%S")+ " [в описании видео, "+ f_name + " " + l_name +"] Зарепортил[+] из-за: "+bad_found+"     "+description
                                    print(info)
                                    log_to_tg(info, None, 'https://vk.com/video'+str(owner_id)+'_'+str(id))
                                    ...
                                    if x > 0:
                                        ...
                                        f_name = ug[0]['first_name']
                                        l_name = ug[0]['last_name']
                                        info = datetime.now().strftime("%H:%M:%S")+ " [в комментариях под видео, "+ f_name + " " + l_name +"] Зарепортил[+] из-за: "+bad_found+"    "+text
                                        print(info)
                                        log_to_tg(info, None, 'https://vk.com/video'+str(owner_id)+'_'+str(id))
                                    else:
                                        if if3 == 0:
    ...
    try:
        ...
        if str(id) not in list_id:
            if x > 0:
                    ...
                    f_name = ug[0]['first_name']
                    l_name = ug[0]['last_name']
                    info = datetime.now().strftime("%H:%M:%S")+ " [фото, "+ f_name + " " + l_name +"] Зарепортил[+] из-за: "+bad_found
                    print(info)
                ...
                if id > 651000000:
                        ...
                        f_name = ug[0]['first_name']
                        l_name = ug[0]['last_name']
                        info = datetime.now().strftime("%H:%M:%S")+ " [фото, "+ f_name + " " + l_name +"] Зарепортил[+] из-за недавно созданной страницы"
                        print(info)
                        log_to_tg(info, url, None)
                else:
                    users_get = vk.users.get(user_ids=id, fields="has_photo")
                    if users_get[0]['has_photo'] == 0:
                            ...
                            f_name = ug[0]['first_name']
                            l_name = ug[0]['last_name']
                            info = datetime.now().strftime("%H:%M:%S")+ " [фото, "+ f_name + " " + l_name +"] Зарепортил[+] из-за отсутствия авы"
                            print(info)
                            log_to_tg(info, url, None)
                    else:
                        if if3 == 0:
                            ...
                            if users_get[0]['online'] == 0:
                                    ...
                                    f_name = ug[0]['first_name']
                                    l_name = ug[0]['last_name']
                                    info = datetime.now().strftime("%H:%M:%S")+ " [фото, "+ f_name + " " + l_name +"] Зарепортил[+] из-за оффлайна(возможно взломанный акк или бот)"
                                    print(info)
                                    log_to_tg(info, url, None)
                            else:
                                list_id.append(str(id))
    ...
    try:
        ...
        if str(id) not in list_id:
            if x > 0:
                ...
                f_name = ug[0]['first_name']
                l_name = ug[0]['last_name']
                info = datetime.now().strftime("%H:%M:%S")+ " [текст, "+ f_name + " " + l_name +"] Зарепортил[+] из-за: "+bad_found+"      "+text
                print(info)
    ...
    while True:
        try:
            ...
            if inp1 == "1":
                ...
                print("Вставьте id группы С МИНУСОМ...")
                group_id = input()
            else:
                print("Введите точные данные")
